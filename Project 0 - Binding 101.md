...
What's important to know about XAML is that the nodes in XAML not only represent UI elements, but they are also directly linked to types defined in the Common Language Runtime (CLR). This means XAML nodes share types with C# and other languages that are powered by the CLR. While the technical specifications of this are far more complex than this summary gives them credit for, the takeaway from XAML nodes being linked to types in the CLR is that the XAML you wrote to define the view will have no trouble communicating with the C# code that you wrote to define the viewmodel.
...
The idea of binding will make sense the more you do it and read code involving it, so don't be intimidated. Move on to Project 1!